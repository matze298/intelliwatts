...
Use short commit prefixes like `feat:`, `fix:`, and `docs:`. Keep commits small and focused, with branch names prefixed `feature/` or `fix/`, not `codex/`. PRs must stay small enough to review in one pass; split oversized work into stacked PRs. Always use `.github/pull_request_template.md` when opening a PR. Include the change summary, migration or asset rebuild notes when relevant, and screenshots for UI work. Update `README.md` and `docs/improvement-plan.md` when setup, usage, or the plan changes.
